...
The current agent-tools version is `1.0.6`. The CLI reports it through `derived --version`, and the MCP server reports the same value in its initialization handshake.
...
```sh
scripts/package-agent-tools.sh 1.0.6
```
