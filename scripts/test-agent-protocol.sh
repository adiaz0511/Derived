...
[[ "$("$build_path/derived" --version)" == "derived 0.2.0" ]]
...
/usr/bin/grep -q 'current CLI and MCP agent-tools version is `0.2.0`' "$project_root/README.md"
...
/usr/bin/grep -q 'current agent-tools version is `0.2.0`' "$project_root/docs/AGENT_INTEGRATIONS.md"
...
assert messages[0]["result"]["serverInfo"]["version"] == "0.2.0"
