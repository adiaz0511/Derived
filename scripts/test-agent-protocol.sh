...
[[ "$("$build_path/derived" --version)" == "derived 1.0.2" ]]
...
/usr/bin/grep -q 'CLI_%2B_MCP-1.0.2' "$project_root/README.md"
...
/usr/bin/grep -q 'current agent-tools version is `1.0.2`' "$project_root/docs/AGENT_INTEGRATIONS.md"
...
assert messages[0]["result"]["serverInfo"]["version"] == "1.0.2"
